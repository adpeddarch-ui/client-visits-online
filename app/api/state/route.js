...
export async function PUT(request) {
  ...
  if (Array.isArray(body.visits)) {
    ...
    const invalidVisit = normalizedVisits.find((item) => validationError(item));
    if (invalidVisit) {
      return NextResponse.json({ error: validationError(invalidVisit) }, { status: 422 });
    }

    try {
      for (const visit of normalizedVisits) {
        const rows = await supabaseFetch("visits", `?id=eq.${encodeURIComponent(visit.id)}&select=*`, {
          method: "PATCH",
          headers: { prefer: "return=representation" },
          body: JSON.stringify(visit),
        });
        ...
        if (!rows.length) {
          const { id, ...newVisit } = visit;
          await supabaseFetch("visits", "?select=*", {
            method: "POST",
            headers: { prefer: "return=representation" },
